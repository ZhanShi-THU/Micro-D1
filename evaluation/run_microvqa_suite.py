...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a packed MicroVQA evaluation suite across phase checkpoints and ablations."
    )
    parser.add_argument(
        "--suite-config",
        type=str,
        default="evaluation/suites/microvqa_phase_progression.yaml",
        help="YAML file describing the evaluation suite.",
    )
    parser.add_argument(
        "--device",
        type=str,
...
def evaluate_modular_run(
    *,
    ...
    for sample in samples:
        ...
        bucket_correct[bucket] = bucket_correct.get(bucket, 0) + int(pred == answer)
...
def make_leaderboard_row(summary: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "object_count_accuracy": summary.get("bucket_accuracy", {}).get("object_count"),
        "attribute_accuracy": summary.get("bucket_accuracy", {}).get("attribute"),
        "other_accuracy": summary.get("bucket_accuracy", {}).get("other"),
...
def main() -> None:
    ...
    leaderboard_rows = sorted(
        leaderboard_rows,
        key=lambda row: (
            -1.0 if row["overall_accuracy"] is None else -float(row["overall_accuracy"])
        ),
    )
