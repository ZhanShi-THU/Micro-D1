...
from data.unified_vqa import format_choices_for_prompt, get_unified_manifest_paths
from models.modular_vlm import ModularVLM


ANSWER_REGEX = re.compile(r"answer is\s*\*?\*?\(?([0-9]+)\)?", re.IGNORECASE)

BASELINE_PROMPT_TEMPLATE = """The following is a multiple choice question (with answers) related to the image below.
<image>
...
def build_baseline_prompt(question: str, choices: Sequence[str]) -> str:
...
class ModularVLMEvaluator:
    def __init__(
        ...
        self.config = config
        self.device = device
        self.model = ModularVLM(config).to(device)
        self.model.eval()
...
def evaluate_finegrained(args: argparse.Namespace, config: Dict[str, Any], device: torch.device) -> None:
    ...
    for sample in samples:
        ...
        modular_prompt = build_microvqa_prompt(sample["question"], sample["choices"])
